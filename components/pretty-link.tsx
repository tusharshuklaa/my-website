...
export const PrettyLink: FC<PrettyLinkProps> = ({ children, className, isExternal = true, minimal = false, ...props }) => {
  ...
  return (
    <Link {...props} href={props.href} className={prettyLinkClasses}>
      ...
      {
        isExternal && <ArrowUpRight className="w-4 h-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
      }
    </Link>
  );
};
