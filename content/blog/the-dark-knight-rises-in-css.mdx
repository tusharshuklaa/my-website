...
- `src/styles` - A directory containing CSS files that incrementally build the Batman image starting from `01.css` upto `9.css`.

I&apos;d recommend you to checkout the repo and follow along for best understanding.
...
Before we begin, scroll up to the top and look at the image of Batman. Notice how the face is a simple trapazoid shape.
A trapezoid is like a rectangle that has at least one pair of parallel sides, making it look like a table top when viewed from the side. We can create this shape using CSS's border property in a clever way.
...
<br />
<br />
Now that we know how to create a trapezoid, let's see how can we use this trick in our batman face and start building the face of our Dark Knight.
...
The trick is to define __ONE and ONLY ONE__ size of the base shape and derive all other sizes from it. There is no specific rule to define the base shape, but it should be the most prominent shape in the image. In this case, the face of Batman is the most prominent shape, so I've defined the width and height of the face as the base shape.
...
<br />
<br />
Now, let&apos;s focus on the trapezoid trick we learned earlier. I've used the same technique to create the head of Batman.
I gave border-bottom a solid color (black) with the width of the desired height of the face. The left and right borders are transparent
to create the slanted sides of the trapezoid. The `--bat-face-side` variable is used to control the slant of the trapezoid.

<br />
<br />
By now, we should have something like this:
![Batman face with trapezoid](batman-step-2_u9ke5g "Batman face with trapezoid")
...
*Creating a traingle* is very similar to creating a trapezoid. This can be achieved by setting the height __AND__ width of the element to 0 while retaining the trapezoind logic.

I&apos;ve used psuedo elements to create ears. Psuedo elements are like invisible elements that you can add to any element. They are very useful for adding decorative elements to your page without cluttering your HTML.
...
### Adding Eyes (04.css)
The next prominent shape in Batman's face is his eyes. Batman's eyes are simple traingles. We can create this using the same logic as ears.
...
<br />
Also notice that instead of creating two separate elements for the eyes, I've used a single element and created two pseudo-elements to represent the eyes. This is a great way to keep your HTML clean and organized.

<br />
<br />
The Dark Knight can see now:
![Batman face with eyes](batman-step-4_qfhthd "Batman face with eyes")
...
Notice the `/` keyword in `border-radius` syntax. This is used to define the horizontal and vertical radii of the corners. I've covered this in detail in tips and tricks section below.
<br />
I've also used `box-shadow` to create the illusion of curves in the cape. The `box-shadow` property is a versatile tool that allows you to add shadows to elements. By cleverly using multiple values in `box-shadow`, you can create a variety of visual effects, including curves, shadows, and highlights.
<br />
Lastly, I've used the `transform: scaleX(-1)` property to flip the right side of the cape horizontally. This creates a mirror image of the left side, giving the cape a symmetrical appearance.
<br />
<br />
Batman is now ready to take on the night:
![Batman face with cape](the-dark-knight-rises-in-css "Batman face with cape")
...
<br />
Notice how I've used the `calc()` function to calculate the reduced height of the eyes based on the width of the face. I've not re-written the logic for height of the eye but have re-calculated the CSS variable that was being used for height of the eye. NEAT!! right?
<br />
Since the eyes were made using a triangle shape, I've changed the border color to red to show Batman's anger. I've also used the `margin-top` property to move the eyes up a bit to give the impression that they are shrinking.
<br />
<br/>
